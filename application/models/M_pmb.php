...
class M_pmb extends CI_Model
{
    ...
    public function pendaftarBank()
    {
        ...
        $this->db->query("SELECT COUNT(p.id_bank) as jumlah, sum(p.nominal_bayar) as total, p.id_bank, p.is_bayar, bank.bank FROM pendaftar p join bank ON p.id_bank=bank.id_bank 
        WHERE p.id_jalur IN (1,2)
        GROUP BY p.id_bank, p.is_bayar");
        $result = $this->db->get('pendaftar')->result_array();
        return $result;
    }
